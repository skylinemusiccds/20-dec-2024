...
class DataStoreManager(private val settingsDataStore: DataStore<Preferences>) {
        ...
        val sendBackToGoogle =
            settingsDataStore.data.map { preferences ->
                preferences[SEND_BACK_TO_AUDIOFLARE] ?: TRUE
            }
        ...
        suspend fun setSendBackToGoogle(send: Boolean) {
            withContext(Dispatchers.IO) {
                if (send) {
                    settingsDataStore.edit { settings ->
                        settings[SEND_BACK_TO_AUDIOFLARE] = TRUE
                    }
                } else {
                    settingsDataStore.edit { settings ->
                        settings[SEND_BACK_TO_AUDIOFLARE] = FALSE
                    }
                }
            }
        }
        ...
        companion object Settings {
            ...
            val SEND_BACK_TO_AUDIOFLARE = stringPreferencesKey("send_back_to_google")
